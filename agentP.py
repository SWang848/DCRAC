...
class DCRACAgent:
    ...
    def policy_update(self, update_actor=True):
        ...
        for i, (_, action, reward, _, done, _, _) in enumerate(batch):
            ...
            if update_actor:
                action_objective[i][action] = self._get_action_objective(q_true[i][action], q_predict[i], p_predict[i], action, w_batch[i])
    ...
    def _get_action_objective(self, q_true_a, q_predict, p_predict, action, weight=None, scalar=True):
        if self.obj_func == 'a':
            # based on advantages
            objective = q_true_a - np.average(q_predict, weights=p_predict, axis=0)
        elif self.obj_func == 'am':
            objective = q_true_a - q_predict.mean(axis=0)
        elif self.obj_func == 'td':
            # based on td error
            objective = q_true_a - q_predict[action]
        ...
        if scalar and weight is not None:
    ...
    def update_priorities(self, batch, ids, ignore_dup=False, pr=False):
        ...
        # q_predict, q_target, p_predict, p_target, w_batch, o_batch, a_last_batch
        q_predict, q_target, p_predict, p_target, _, _, _ = self._get_training_data(batch)
        ...
        for i, (_, action, reward, _, terminal, _, _) in enumerate(batch):
            ...
            # error = np.abs(self._get_action_objective(target, q_predict[i], p_predict[i], action, scalar=False)).mean()
            errors[i] = error
    ...
    def memorize(self, state, action, reward, next_state, terminal, action_prev, acts_prob, 
        initial_error=.99, trace_id=None, pred_idx=None):
